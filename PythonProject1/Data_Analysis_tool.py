...
def save_file(df, original_file):
    ...
    try:
        ...
        new_location = input("Enter a new location and file name to save the file (e.g., C:\\Users\\Mayank\\Documents\\new_file.csv): ")
...
def clean_data(df, original_file):
    ...
    try:
        ...
        if choice == 1:
            ...
            df.columns = df.columns.str.strip().str.lower().str.replace(' ', '').str.replace(r'[^a-zA-Z0-9]', '', regex=True)
...
# Main Program
def main():
    folder = "C:\\Users\\Prashant Khatri\\Downloads" # Folder containing the files
    df = None
    original_file = None
